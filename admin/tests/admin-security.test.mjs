...
class FakeRepository {
  committed;
  constructor() {
    this.blobs = { schema: PLACE_SCHEMA, narrativeSchema: NARRATIVE_SCHEMA, commonSchema: COMMON_SCHEMA, mediaSchema: MEDIA_SCHEMA, feastSchema: FEAST_SCHEMA, feastRegistry: FEAST_REGISTRY, preview: PREVIEW, place: EXISTING, narrative: NARRATIVE };
  }
  ...
  async readTree() {
    return [
      ...
      { path: "schemas/feast-registry.schema.json", mode: "100644", type: "blob", sha: "feastSchema" },
      { path: "content/feasts/registry.yaml", mode: "100644", type: "blob", sha: "feastRegistry" },
...
const validBody = {
  ...
  patronalFeastIds: [],
  stagedFeasts: [],
  expectedFeastRegistryBlobSha: "feastRegistry",
};
...
async function assertSafeGitHubAuthenticationFailure(error) {
...
test("incomplete GitHub App configuration stays private", async () => {
  const repository = new GitHubRepository({
    env,
...
test("GitHub installation token failures keep internal diagnostics out of responses", async () => {
  const { privateKeyPem } = createGitHubAppKeyPair("pkcs8");
  ...
  await assertSafeGitHubAuthenticationFailure(networkError);
});
...
test("GitHub authentication error serialization omits all diagnostic fields", async () => {
...
test("internal GitHub diagnostics are omitted from public responses", async () => {
