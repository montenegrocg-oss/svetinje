...
import { relatedPlacesFor } from "./related-places";
...
export async function localizedStaticPaths(locale: "ru" | "en") {
  ...
  return [...staticPaths, ...places.map((place) => ({
    params: { path: `${placePrefix}/${place.slug}` },
    props: { locale, page: "place", place, relatedPlaces: relatedPlacesFor(place, places, locale), routes: [] },
  }))];
}
