...
test("the explorer keeps one shared filter and pagination state across cards, controls, and map markers", async () => {
  ...
  assert.match(explorer, /const filteredCards = placeCards\.filter/);
  assert.match(explorer, /const matches = matchesArea && matchesCategory/);
  assert.match(explorer, /const rankedCards = rankHomepageDiscoverySearch/);
  ...
  assert.match(explorer, /matchedCards = rankedCards\.map/);
