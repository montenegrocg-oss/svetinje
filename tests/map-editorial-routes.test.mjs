...
test("walking-route copy and map behavior are localized without joining marker filters", async () => {
  const [canvas, controls, explorer] = await Promise.all([
  ...
  assert.match(controls, /hasEditorialRoutes && <button[^>]*data-route-toggle/);
